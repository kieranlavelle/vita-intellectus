...
<main>
    <div class="flex flex-col justify-center min-h-screen min-w-full items-center bg-gray-50">
        <div class="max-w-md w-full space-y-8"></div>
            <div>
                <h1 class="text-3xl font-extrabold text-gray-900 text-center">Log in</h1>
                <p class="mt-2">
                    Or <a class="text-md text-indigo-600 hover:text-indigo-700">register for an account</a>
                </p>
            </div>
            {#if errorMessage != ''}
            ...
            {/if}
            <form on:submit|preventDefault={login} class="flex flex-col space-y-6 mt-8">
